...
既然有了代码训练，为什么还要做问答题呢？主要有两点考量。

第一点考量是大模型的使用。众所周知现在 vibe coding 十分风靡，一方面学生很容易依赖 vibe coding 而缺少思考，因此笔者在每一节最后都设置了有关 AI 使用的问答题，引导同学对大模型生成质量的好坏进行思考和评判；且 vibe coding 出现的时间也不算长，笔者亦不能得知 vibe coding 对编程新手的影响到底有多大，这个问题亦可视为一次社会调查，调查大语言模型对新手的学习究竟有怎样的影响。

第二点是关于代码框架本身。由于代码框架中给出了大部分的输入输出等实现，尤其是在第一章中给出的框架更是完善，同学们只需要写一点代码即可完成。这么设计，一方面是考虑到同学们刚刚接触 .NET 框架，如果让同学们过多地自己写，那么具有一定难度（可以看到框架给出的部分是逐章减少的，以适应同学们的学习曲线），另一方面是前面的基础章节是后面章节的基础，尽量让同学们不出太大错误，也不想导致同学们写不出来，否则会影响同学们后续章节的学习和发挥。但这样又没办法保证同学们深入地理解了代码框架以更好地学习到本章的内容。因此，一部分问答题就代码框架的实现进行提问，以引导同学们对代码框架进行阅读。
