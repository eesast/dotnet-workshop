...
namespace LogParser.Parser
{
    internal static class LineParser
    {
        ...
        private static LogEntry CreateInternal(LogRecord logRecord)
        {
            ...
            var index=internalMessage.Exception.IndexOf(": ");
            var exceptionMessage=internalMessage.Exception.Substring(index+2);
            var exceptionName=internalMessage.Exception.Substring(0,index);
