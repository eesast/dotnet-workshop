...
namespace LogParser.Parser
{
    internal static class LineParser
    {
        ...
        private static LogEntry CreateInternal(LogRecord logRecord)
        {
            ...
            var separatorIndex = internalMessage.Exception.IndexOf(": ", StringComparison.Ordinal);
            if (separatorIndex < 0)
            {
            ...
            return new InternalLogEntry(
                ...
                ExceptionName: internalMessage.Exception.Substring(0, separatorIndex),
                ExceptionMessage: internalMessage.Exception.Substring(separatorIndex + 2)
            );
        }
