...
namespace LogParser.Parser
{
    internal static class LineParser
    {
        ...
        private static LogEntry CreateInternal(LogRecord logRecord)
        {
            ...
            // 字段值固定为 "ExceptionName: exception message" 格式，按第一个冒号+空格切分
            var separatorIndex = internalMessage.Exception.IndexOf(": ");
            if (separatorIndex < 0)
            {
                throw new FormatException($"Exception field is not in 'ExceptionName: message' format: {internalMessage.Exception}");
            }
            return new InternalLogEntry(
                ...
                ExceptionName: internalMessage.Exception[..separatorIndex],
                ExceptionMessage: internalMessage.Exception[(separatorIndex + 2)..]
            );
        }
