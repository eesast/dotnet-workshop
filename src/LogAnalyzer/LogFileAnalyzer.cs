    ...
    public class LogFileAnalyzer
    {
        ...
        private void RunWorkers(int degreeOfParallelism, IReadOnlyList<FileInfo> fileList)
        {
            ...
            lock (_syncRoot)
            {
                ...
                {
                    if (!_analysisResults.TryGetValue(file.Name, out var analysisResult))
                    {
                        throw new InvalidOperationException($"Unknown file: {file.Name}.");
                    }
                    if (analysisResult.State == AnalysisState.NotAnalyzed)
                    {
            ...
            for (int i = 0; i < degreeOfParallelism; i++)
            {
                ...
                var worker = new Thread(() => WorkerMain(workerId, queue))
                {
                    IsBackground = true,
                    Name = threadName,
                };
